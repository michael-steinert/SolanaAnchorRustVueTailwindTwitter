...
const fetchTopicTweets = async () => {
  if (slugTopic.value === viewedTopic.value) {
    return;
  }
  try {
    loading.value = true;
...
<!-- The Topics Page allows Users to enter a Topic and displays all Tweets matching it -->
<!-- Once a Topic is entered it also displays a Form to send Tweets with that Topic pre-filled -->
<template>
  <tweet-search
      placeholder="topic" :disabled="! slugTopic" :modelValue="slugTopic"
