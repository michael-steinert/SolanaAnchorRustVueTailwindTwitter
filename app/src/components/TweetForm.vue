...
/* Form Data of Component */
const content = ref("");
const topic = ref("");
const slugTopic = useSlug(topic);
const effectiveTopic = computed(() => {
  return (forcedTopic.value ?? slugTopic.value);
});
...
const characterLimitColour = computed(() => {
  ...
  if (characterLimit.value <= 10) {
    return ("text-yellow-500");
  }
  return ("text-gray-400");
});

/* Permissions of Component */
const {connected} = useWallet();
const canTweet = computed(() => {
  return (content.value && characterLimit.value > 0);
});

/* Actions of Component */
...
<!-- The TweetForm Component designs the Form allowing Users to send Tweets -->
<!-- It contains a Field for the Content, a Field for the Topic and a little Character Count-Down -->
<template>
  ...
  <div v-else class="px-8 py-4 bg-gray-50 text-gray-500 text-center border-b">
    Connect Wallet to start Tweeting
  </div>
</template>
